...
import os
import yaml
from algorithm.movie_QA_system.config import root_path
import torch
import torch.optim as optim
from algorithm.movie_QA_system.NER.data_manager import DataManager
from algorithm.movie_QA_system.NER.model import BiLSTMCRF
from algorithm.movie_QA_system.NER.utils import f1_score, get_tags, format_result


class ChineseNER(object):
    
    def __init__(self, entry="train"):
        self.cur_dir = os.path.join(root_path, 'NER')
        self.load_config()
    ...
    def load_config(self):
        try:
            fopen = open(os.path.join(self.cur_dir,'models/config.yml'))
        ...
        self.model_path = os.path.join(self.cur_dir,config.get("model_path"))
    ...
    def save_params(self, data):
        with open(self.model_path + "data.pkl", "wb") as fopen:
            pickle.dump(data, fopen)

    def load_params(self):
        with open(self.model_path + "data.pkl", "rb") as fopen:
