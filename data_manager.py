...
class DataManager():
    def __init__(self, max_length=100, batch_size=20, data_type='train', tags=[]):
        ...
        self.path = os.path.join(root_path,'NER/data')

        if data_type == "train":
            assert tags, Exception("请指定需要训练的tag类型，如[\"ORG\", \"PER\"]")
            self.generate_tags(tags)
            self.data_path = os.path.join(self.path, "train_ner")
        elif data_type == "dev":
            self.data_path = os.path.join(self.path, "dev")
            self.load_data_map()
        elif data_type == "test":
    ...
    def load_data_map(self):
        with open(os.path.join(root_path,"NER/models/data.pkl"), "rb") as f:
